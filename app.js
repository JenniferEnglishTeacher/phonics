...
function imageSearchTerm(item) {
  return (item.answer || item.word || "").replace(/\(.*?\)/g, "").trim();
}

function imageMeaning(item) {
  return (item.definition || "").replace(/[;；/].*$/, "").trim();
}

function englishMeaning(item) {
  const sentence = sentenceWithAnswer(item);
  const answer = imageSearchTerm(item);
  if (!answer || !sentence) return answer;
  const cleanSentence = sentence.replace(new RegExp(`\\b${answer.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`, "i"), "").trim();
  return `${answer} ${cleanSentence}`.replace(/\s+/g, " ").slice(0, 90);
}

function isOpenLicense(license) {
  return /cc0|public domain|cc by|cc-by|creative commons/i.test(license || "");
}

function imageQueries(item) {
  const term = imageSearchTerm(item);
  const meaning = imageMeaning(item);
  const english = englishMeaning(item);
  return [
    `${english} ${meaning} cartoon`,
    `${english} ${meaning} cartoon illustration`,
    `${term} ${meaning} cartoon`,
    `${term} ${meaning} cartoon clipart`,
    `${english} cartoon`,
    term
  ].filter(Boolean);
}

function imageScore(image, item) {
  const text = normalizeText(`${image.title} ${image.description} ${image.categories}`);
  const term = normalizeText(imageSearchTerm(item));
  const meaning = normalizeText(imageMeaning(item));
  let score = 0;
  if (term && text.includes(term)) score += 10;
  if (meaning && text.includes(meaning)) score += 12;
  if (/cartoon|illustration|clipart|drawing|icon|svg|png|vector|child|children/.test(text)) score += 8;
  if (/logo|flag|map|diagram|seal|coat of arms|emblem/.test(text)) score -= 10;
  return score;
}

async function fetchCommonsImages(item) {
  const cacheKey = `${imageSearchTerm(item)}|${imageMeaning(item)}`;
  if (state.imageCache.has(cacheKey)) return state.imageCache.get(cacheKey);
  const found = [];
  const seen = new Set();
  for (const query of imageQueries(item)) {
    ...
    try {
      ...
      pages.forEach(page => {
        ...
        if (image.url && isOpenLicense(image.license) && !seen.has(image.url)) {
          seen.add(image.url);
          found.push(image);
        }
      });
    } catch {
      // Try the next search phrase.
    }
  }
  found.sort((a, b) => imageScore(b, item) - imageScore(a, item));
  state.imageCache.set(cacheKey, found);
  return found;
}

function renderCardImage(card, item, images) {
  const imageSlot = card.querySelector(".photo-slot");
  const credit = card.querySelector(".photo-credit");
  const index = Number(card.dataset.imageIndex || 0);
  const image = images[index % images.length];
  if (!image) {
    imageSlot.innerHTML = `<span>No CC cartoon/photo found for ${escapeHtml(imageSearchTerm(item))} ${escapeHtml(imageMeaning(item))}.</span>`;
    credit.textContent = "";
    return;
  }
  imageSlot.innerHTML = `<img src="${escapeHtml(image.url)}" alt="${escapeHtml(imageSearchTerm(item))} ${escapeHtml(imageMeaning(item))}">`;
  credit.textContent = `${image.license || "CC"} / Wikimedia Commons${image.artist ? ` / ${image.artist}` : ""}`;
}

async function showCardImage(card, item) {
  const imageSlot = card.querySelector(".photo-slot");
  if (!imageSlot) return;
  if (imageSlot.dataset.loaded === "true") {
    const images = await fetchCommonsImages(item);
    renderCardImage(card, item, images);
    return;
  }
  imageSlot.textContent = "Loading photo...";
  const images = await fetchCommonsImages(item);
...
function renderWords(rule) {
  ...
  rule.items.forEach(item => {
    ...
    card.innerHTML = `
      <div class="flip-card">
        ...
        <div class="flip-face flip-back">
          <div class="photo-slot"><span>Loading photo...</span></div>
          <p class="photo-word">${escapeHtml(item.word)}</p>
          <p class="photo-credit"></p>
          <button type="button" data-next-photo>Next picture</button>
        </div>
      </div>
    `;
    card.addEventListener("click", () => {
      card.classList.toggle("flipped");
      if (card.classList.contains("flipped")) showCardImage(card, item);
    });
    card.querySelector("[data-speak-word]").addEventListener("click", event => {
    ...
    card.querySelector("[data-next-photo]").addEventListener("click", async event => {
      event.stopPropagation();
      const images = await fetchCommonsImages(item);
      card.dataset.imageIndex = String((Number(card.dataset.imageIndex || 0) + 1) % Math.max(images.length, 1));
      renderCardImage(card, item, images);
    });

...
async function fetchCommonsImage(item) {
  const term = imageSearchTerm(item);
  if (state.imageCache.has(term)) return state.imageCache.get(term);
  ...
  const empty = { url: "", title: "", license: "", artist: "", page: "" };
  try {
    ...
    const match = pages
      .map(page => {
        ...
        return {
          ...
          page: meta.ImageDescription?.source || info?.descriptionurl || ""
        };
      })
      .find(image => image.url && isOpenLicense(image.license));
    const result = match || empty;
    state.imageCache.set(term, result);
    return result;
  } catch {
    state.imageCache.set(term, empty);
    return empty;
  }
}
...
async function showCardImage(card, item) {
  ...
  if (!imageSlot || imageSlot.dataset.loaded === "true") return;
  imageSlot.textContent = "Loading photo...";
  const image = await fetchCommonsImage(item);
  imageSlot.dataset.loaded = "true";
  if (!image.url) {
    imageSlot.innerHTML = `<span>No CC photo found for ${escapeHtml(imageSearchTerm(item))}.</span>`;
    credit.textContent = "";
    return;
  }
  imageSlot.innerHTML = `<img src="${escapeHtml(image.url)}" alt="${escapeHtml(imageSearchTerm(item))}">`;
